            ...
            if person.origin_from.count() > 0:
                ...
                for idx, origin in enumerate(origins):
                    ...
                    if idx == 0 or idx == origins.count() - 1:
                        ...
                        properties["origin_{}".format(pos)] = origin.place.address
                        properties[
                            "origin_{}_type".format(pos)
                        ] = origin.origin_type.title
                        ...
                        properties["origin_{}_date".format(pos)] = "{} 00:00".format(
                            origin.date
                        )
                        properties[
                            "origin_{}_is_date_computed".format(pos)
                        ] = origin.is_date_computed

                    if origin.place != prev_place:
                        ts = datetime.fromordinal(origin.date.toordinal()).timestamp()
                        coords.append(
                            [
                                float(origin.place.lon),
